...
\begin{document}
	...
	\begin{frame}
		...
		\begin{figure}[h!]
			...
			\caption{Source: \cite{mirex_website_2015_results}}
		\end{figure}
	\end{frame}
	...
	\subsection{NCD mit LZF}
	\begin{frame}
		\frametitle{NCD mit LZF}
		\begin{minipage}{0.45\textwidth}
			\begin{figure}[h!]
				\fbox{\includegraphics[width=100px,height=100px,keepaspectratio,page=1]{SNT1}}
				\footnotemark
			\end{figure}
		\end{minipage}%
		\begin{minipage}{0.45\textwidth}
			\begin{itemize}
				\item Das Alphabet besteht aus 13 Symbolen
				\note[item]<1>{12 Symbole werden zur Darstellung der Noten eingesetzt und ein Symbol für die Pause}
				\item $SIM_s(x,y) = max\{s(x \oplus d , y) | d = 0, \dots ,11 \}$
			\end{itemize}
		\end{minipage}
		\footnotetext[1]{
			\textit{"MIREX 2015 Symbolic Melodic Similarity : Applying String Similarity Measures to Pitch/Rest Sequences"}
			\cite{MIREX_2015_one} von Shiho Sugimoto, Yuto  Nakashima, Masayuki Takeda.
		}
	\end{frame}

	\begin{frame}
		\begin{figure}[h!]
				\includegraphics[width=150px,height=150px,keepaspectratio]{one_of_MIREX_2015_one}
		\end{figure}
		\begin{itemize}
			\item $K(\cdot)$ - Kolmogorov-Komplexität
			\item $K(\cdot | \cdot)$ - bedingte Kolmogorov-Komplexität
			\item $K(x|y) \approx K(yx) - K(y)$
			\item K(x) wird dann komprimiert zu C(x)
		\end{itemize}
		\begin{figure}[h!]
				\includegraphics[width=150px,height=150px,keepaspectratio]{two_of_MIREX_2015_one}
		\end{figure}
	\end{frame}

	\begin{frame}
		\frametitle{LZ-Factorisierung}
		\begin{itemize}
			\item Lempel-Ziv Algorithmus
			\item Sei $T$ ein String. Die Faktorisierung dieses Strings ist dann definiert als $T=f_1\dots f_n$ , wo jeder Faktor $f_k \in \Sigma^{+} (k=1,\dots,n)$ induktiv definiert ist als:
			\begin{flalign*}
				&f_1 = T[1] \text{erster Index }\\
				&f_k = 
				\begin{cases}
					T[|f_1\dots f_{k-1}| +1] = c \in \Sigma &,\text{wenn $\{c\} \subsetneq f_1\dots f_{k-1}$}\\
					lcp_{\geq2}(f_k , \dots , f_n) &, \text{sonst.}\\\end{cases}
			\end{flalign*}
			Dabei heißt $lcp_{\geq2}$ die Longest-Common-Präfix , die mindestens zweimal in $f_1\dots f_k$ vorkommt.
		\end{itemize}
	\end{frame}

	\begin{frame}
		\frametitle{FLDC Kernel}
			\begin{itemize}
			\item Seien $m$ und $k$ fest , wobei $m>0$ und $0\leq k \leq m$.
			\end{itemize}
			\[
				F_{(m,k)} = \{q | q \in (\Sigma \cup \{\#\})^m \text{ und } \#(q) \leq k \}
			\]

			$\#(q) =^{\textit{df.}} \text{Anzahl an $\#$ in q}$ 

			\begin{figure}[h!]
				\fbox{\includegraphics[width=200px,height=200px,keepaspectratio]{three_of_MIREX_2015_one}}
			\end{figure}
	...
	\begin{frame}[allowframebreaks]
		...
		\begin{thebibliography}{5}
			...
			\bibitem[5]{mirex_website_2015_results} MIREX,Symbolic Melodic Similarity Results 2015, https://www.music-ir.org/mirex/wiki/2015:Symbolic\_Melodic\_Similarity\_Results.
            ...
            \bibitem[11]{mirex_2005_one} Grachten, Maarten \& Arcos, Josep Lluís \& Mántaras, Ramon. (2020). Melody Retrieval using the Implication/Realization Model. 
           	...
           	\bibitem[14]{MIREX_2015_one} Sugimoto  Shiho, Nakashima Yuto, Takeda Masayuki, "MIREX 2015 Symbolic Melodic Similarity : Applying String Similarity Measures to Pitch/Rest Sequences". Technical report, Music Information Retrieval Evaluation eXchange, 2015
		\end{thebibliography}
	\end{frame}
\end{document}
